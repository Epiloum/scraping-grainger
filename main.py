...
def get_grainger_categories(categories):
    for item in categories:
        ...
        if len(res):
            ...
            item['layout'] = check_layout(html)
            item['variants'] = get_variants(html)
...
def get_variants(html):
    ...
    attributes = data['category']['category']['attributes']

    singular = [item['name'] for item in attributes if (item['visible'] and not item['splitMultiValues'])]
    multiple = [item['name'] for item in attributes if (item['visible'] and item['splitMultiValues'])]
    return {'singular': singular, 'multiple': multiple}
...
def export_categories_to_table(head, data):
    ...
    for item in data:
        if 'subs' in item:
            ...
            categories = head + '<td><a href="https://www.grainger.com' + item['url'] + '">' + item['title'] + '</a></td>' + ('<td></td>' * (8 - depth))
            layout = '<td>' + item['layout'] + '</td>'
            variants_singular = ''.join(['<td>' + s + '</td>' for s in item['variants']['singular']]) +  ('<td></td>' * (12 - count_singular))
            variants_multiple = ''.join(['<td>' + s + '</td>' for s in item['variants']['multiple']]) +  ('<td></td>' * (4 - count_multiple))
...
#tar = [{'url':'/category/office-supplies/calendars-planners', 'title': 'Calendars & Planners'}]
